...
describe("R-CDE の PCLOD バッチ処理ステータスとの同期（BATCH_PROCESSING_STATUS / isBatchProcessingStatus）", () => {
  describe("正常系", () => {
    it("R-CDE の server/pkg/ent/schema/batchprocessingresult.go と同じ 開始:1 / 進行中:2 / 完了:3 / 失敗:4 を持つ", () => {
    ...
    it("R-CDE が定義している 1 から 4 までの値は、既知のステータスとして受け入れる", () => {
      expect([1, 2, 3, 4].every(isBatchProcessingStatus)).toBe(true);
    });
  });

  describe("異常系", () => {
    it("R-CDE に存在しない数値は、既知のステータスとして受け入れない", () => {
