...
describe("resolveViewerMemoryObservedBytes", () => {
  it("estimate / jsHeap / page の3値を常に返す", () => {
...
describe("resolveViewerMemoryTargetBytes", () => {
  const observedBytes = resolveViewerMemoryObservedBytes(baseSample);

  it("対象ごとの値を返す", () => {
    expect(resolveViewerMemoryTargetBytes(observedBytes, "estimate")).toBe(200);
    expect(resolveViewerMemoryTargetBytes(observedBytes, "jsHeap")).toBe(250);
    expect(resolveViewerMemoryTargetBytes(observedBytes, "page")).toBe(300);
  });
...
describe("evaluateViewerMemoryAlert", () => {
  ...
  it("対象ごとに異なるレベルを判定し、最も高いレベルをアラートレベルにする", () => {
    const result = evaluateViewerMemoryAlert({
      sample: baseSample,
      thresholds: {
