...
レフト/ライトサイドバーから切り出した機能（`useContractFileActions` フック）の動作確認項目。

- 確認場所: 利用側アプリのファイル一覧 UI
...
`getFileStatus` は日本語ラベルではなく状態値（`upload`: `uploading` / `uploaded`、`pclod`: `none` / `waiting` / `processing` / `completed` / `failed` / `unknown`）を返す。文言は利用側の写像で決まる。
