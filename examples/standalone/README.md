...
- 各行のファイル名の下に PCLOD 処理・アップロード状況のチップを表示（下記参照）
- 各ファイルの目アイコンで **表示 / 非表示を切り替え**（PCLOD 完了後のみ操作可能）
- フォーカスアイコンでそのファイルの中心へ基準点を移動

## ファイルステータスチップの確認

SDK の `getFileStatus` は日本語の文言ではなく状態値を返します。本サンプルでは `ContractFileSidebar.tsx` の `UPLOAD_STATUS_LABELS` / `PCLOD_STATUS_LABELS` で文言に写像しています。
