...
重複の判定は一覧に載っている 100 件が対象です。一覧から消した通知と、100 件を超えて
押し出された通知は、同じイベント ID で再び受けても重複とは出ません。
...
手で貼った値は tunnel が動いていても上書きされないので、ngrok など別の公開経路も試せます。
自動で拾い直したいときは欄を空にしてください。
